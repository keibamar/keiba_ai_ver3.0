"""
2026年 全場 v3blend / v4 / v5 的中率・回収率 一括集計（新馬戦限定）

実行:
    python scripts/compare_v3_v4_v5_shinba.py
"""
...
print("=" * 90)
print(f"2026年 的中率・回収率集計  【{CLASS_FILTER}戦限定】  全場 v3blend / v4 / v5  (100円/点)")
...
for place_id in active_places:
    ...
    for cp in race_paths:
        ...
        # 新馬戦以外はスキップ
        if str(race_info_df.at[0, "class"]) != CLASS_FILTER:
            filtered += 1; continue
    ...
    n = place_stats[place_name]["v3blend"].n
    if n > 0:
        print(f"\n  --- {place_name} 小計 ({n}R / {CLASS_FILTER}戦) ---")
        for m in MODELS:
...
print(f"\n{'='*90}")
print(f"全場合計: {processed}R処理  スキップ:{skipped}R  対象外:{filtered}R  所要:{elapsed_total:.0f}秒")
print("-" * 90)
