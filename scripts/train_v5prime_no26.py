"""v5prime モデル学習（2026年除外・時系列分割修正版）

  - TRAIN_YEARS: 2020〜2025（2026除外）
  - split: 先頭80%=train（古い）、後ろ20%=validation（新しい）← 時系列的に正しい方向
  - データ: v5のCSVからcurrent_odds/current_popularityを除外（再生成不要）
  - サフィックス: _v5prime_no26

実行:
    python scripts/train_v5prime_no26.py
"""
    ...
    for place_id in TARGET_VENUES:
        ...
        for race_type, length in name_header.COURSE_LISTS[place_id - 1]:
            ...
            try:
                ...
                actual_num = [c for c in race_data.columns if c not in ["race_id"] + CAT_COLS]
                race_data[actual_num] = race_data[actual_num].fillna(-1)
...
if __name__ == "__main__":
    print("=" * 55)
    print(f"v5prime_no26  学習年: {TRAIN_YEARS[0]}〜{TRAIN_YEARS[-1]}")
