"""
2026年 全場 v3blend / v4 / v5 / v5' 的中率・回収率 一括集計

実行:
    python scripts/compare_v3_v4_v5_v5prime.py

- v3blend : v3モデル(64特徴量) + race_result 最終オッズでブレンド
- v4      : v4モデル(67特徴量) の生スコアで順位付け（オッズ・人気追加）
- v5      : v5モデル(70特徴量) の生スコアで順位付け（v4 + 血統カテゴリカル）
- v5'     : v5'モデル(68特徴量) の生スコアで順位付け（v3 + 血統カテゴリカルのみ・オッズ人気なし）
"""
...
def v5prime_ranks(full_df_64, place_id, race_type, course_len, horse_ids):
    """v5'のスコア計算: v3の64特徴量 + 血統3列（オッズ・人気なし）"""
    try:
        father_ids, mf_ids, pgf_ids = [], [], []
        for hid in horse_ids:
            f_cat, mf_cat, pgf_cat = get_pedigree_cats(hid, _VOCAB)
        ...
        full_df_67 = pd.concat([full_df_64.reset_index(drop=True),
                                 extra.reset_index(drop=True)], axis=1)
        model  = _get_model(place_id, race_type, course_len, "_v5prime")
        y_pred = model.predict(full_df_67, num_iteration=model.best_iteration)
        return rank_index(y_pred)
    except Exception as e:
        prediction_error(e)
        return None
...
print("=" * 90)
print("2026年 的中率・回収率集計  全場 v3blend / v4 / v5 / v5'  (100円/点)")
...
for place_id in active_places:
    ...
    for cp in race_paths:
        ...
        if ok:
            horse_ids_list = rc_df["horse_id"].tolist()
            ranks_v5p = v5prime_ranks(full_df_64, pid, rtype, clen, horse_ids_list)
            if ranks_v5p is not None:
                v5p_sorted   = np.argsort(ranks_v5p)
                v5prime_top5 = [str(int(float(rc_df["馬番"].iloc[i]))) for i in v5p_sorted[:5]]

        for mname, top5 in [("v3blend", v3_top5), ("v4", v4_top5),
                              ("v5", v5_top5), ("v5prime", v5prime_top5)]:
