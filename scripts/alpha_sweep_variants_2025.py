"""
v6hit_binary / v6hit_extreme × v5prime のブレンド比率αスイープ

score = α × hit_model_score + (1-α) × v5prime_score

hit_modelを binary / extreme の2種類で比較し、
各αで単勝・複勝・3連複5頭のバックテストを実施する。

実行:
    python scripts/alpha_sweep_variants.py
"""
...
def score_hit(full_df_64, place_id, race_type, course_len, final_odds, final_pop, suffix):
    """v6hit系（オッズ・人気含む66列）"""
    try:
...
for place_id in active_places:
    ...
    for cp in race_paths:
        ...
        s_bin = score_hit(full_df_64, pid, rtype, clen, final_odds, final_pop, "_v6hit_binary")
        s_ext = score_hit(full_df_64, pid, rtype, clen, final_odds, final_pop, "_v6hit_extreme")
        s_v5p = score_v5prime(full_df_64, pid, rtype, clen, rc_df["horse_id"].tolist())

        if s_bin is None or s_ext is None or s_v5p is None: skipped += 1; continue
        ...
        race_records.append({
            ...
            "s_bin": np.array(s_bin, dtype=float),
...
def sweep(label, score_key):
    print(f"\n{'='*100}")
    print(f"【{label}】 × v5prime  αスイープ  (α=1.0がhit単独, α=0.0がv5prime単独)")
    print(f"{'='*100}")
    ...
    for alpha in ALPHAS:
        ...
        for r in race_records:
            blended = alpha * _norm(r[score_key]) + (1 - alpha) * _norm(r["s_v5p"])
...
sweep("v6hit_binary", "s_bin")
sweep("v6hit_extreme", "s_ext")
