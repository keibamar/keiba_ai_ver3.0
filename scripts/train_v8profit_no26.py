"""v8profit_no26 モデル学習スクリプト（回収率直接最適化・カスタムLoss）

目的関数を「回収率最大化」に直接変更する。
勝ち馬への賭けは +(odds-1)×100円 の利益、負け馬は -100円 の損失。
この損益を直接最大化するカスタム勾配をLightGBMに与える。

  損失関数: L = -(odds-1)×log(p) for winner / -log(1-p) for non-winner
  勾配:     g = -(odds-1)×(1-p) for winner / p for non-winner
  ヘッセ:   h = (odds-1)×p×(1-p) for winner / p×(1-p) for non-winner

  ラベルエンコード: y_enc = current_odds for winner, 0 for non-winner
  評価指標: バリデーション期間の本命TOP1単勝シミュレーション回収率（ROI）
  TRAIN_YEARS: 2020〜2025（リークなし）
  サフィックス: _v8profit_no26

実行:
    python scripts/train_v8profit_no26.py
"""
...
MODEL_SUFFIX  = "_v8profit_no26"
N_TRIALS      = 20
ODDS_CAP      = 50.0   # 勾配爆発防止のオッズ上限
...
def profit_objective(y_pred, dataset):
    """回収率最大化カスタム勾配（LightGBM 4.x API: fobj(y_pred, dataset)）。
    dataset.get_label() で y_enc（勝ち馬=current_odds, 非勝ち馬=0）を取得する。
    """
...
def _race_ids_array(race_data_df):
    return race_data_df["race_id"].values


def _fit(params, ds_tr, ds_va, feval_fn, n_estimators):
    # LightGBM 4.x: カスタム目的関数は params["objective"] に callable を渡す
    full_params = {
        "verbosity": -1,
...
def _tune(X_tr, y_tr, X_va, y_va, feval_fn, n_trials=20):
    ...
    def objective(trial):
        ...
        try:
            # Optunaの各試行で Dataset を再生成する（使い回し不可のため）
    ...
    for place_id in TARGET_VENUES:
        ...
        for race_type, length in name_header.COURSE_LISTS[place_id - 1]:
            ...
            try:
                ...
                rid_va   = _race_ids_array(data_va)
...
if __name__ == "__main__":
    print("=" * 55)
    print(f"v8profit_no26  学習年: {TRAIN_YEARS[0]}〜{TRAIN_YEARS[-1]}")
    print("目的関数: 回収率直接最適化（カスタム勾配）")
