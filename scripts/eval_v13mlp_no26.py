...
v13mlp_no26（Embedding + MLP）を v7odds と比較評価。
v12eval キャッシュを流用して v13mlp スコアを追加する。

実行: python scripts/eval_v13mlp_no26.py
"""
...
from src.PredictionModels.LightGBM.make_dataset_v8 import (
    get_extra_past_race_features_v8, index_v8,
)
...
V13_CACHE = os.path.join(PROJECT_ROOT, "logs", f"race_records_v13eval_{EVAL_YEAR}.pkl")
...
def _get_mlp_model(place_id, race_type, length):
    ...
    mp = os.path.join(paths.PREDICTION_MODEL_PATH, PLACE_LIST[place_id - 1],
                      f"{type_str}{length}_lambdarank_model_v13mlp_no26.pt")
    ...
    model = EmbeddingMLP(
        ...
        hidden=ckpt["params"].get("hidden", [256, 128, 64]),
        dropout=ckpt["params"].get("dropout", 0.2),
    ).to(DEVICE)
    model.load_state_dict(ckpt["state_dict"])
    model.eval()
    _mlp_cache[key] = model
    return model
...
def build_v8_row(race_id, rc_df, race_info_df, race_card_path):
    """評価用に v8 特徴量DataFrame（horse単位）を構築する。"""
    try:
        ...
        dataset_v3, extra_v6_rows = pd.DataFrame(), []
        ...
        for hid in horse_ids:
            ...
            try:
                ...
                days_since = np.nan
                if not race_info_5.empty and not pd.isna(current_dt):
                    try:
                        ...
                        pass
                n_horses_1 = _parse_kinryo(race_info_5.iloc[0].get("頭数", "")) if not race_info_5.empty else np.nan
                hw_abs_1   = _parse_weight(race_info_5.iloc[0].get("馬体重", ""))  if not race_info_5.empty else np.nan
            except Exception:
                extra_v6 = [np.nan] * 16
...
def score_v13mlp(full_df_64, v6_extra_df, v7_extra_df, v8_extra_df,
    ...
    try:
        ...
        # v8 フルDataFrame（98列, race_id除く）を組み立てる
        df_full = pd.concat([
            full_df_64.reset_index(drop=True),
            pd.DataFrame({"current_odds": odds_arr, "current_popularity": pop_arr}),
            pd.DataFrame({"father_cat": f_ids, "mother_father_cat": mf_ids, "paternal_gf_cat": pgf_ids}),
        ...
        # 数値列（blood除く）と blood 列を分離
        num_cols = [c for c in df_full.columns if c not in EMBED_COLS]
        X_num = torch.tensor(df_full[num_cols].fillna(-1).values.astype(np.float32),
                             dtype=torch.float32).to(DEVICE)

        def _safe_id(col):
            arr = df_full[col].fillna(-1).values.astype(np.int64)
            arr = np.where(arr < 0, 0, arr)
            arr = np.where(arr > _VOCAB_SIZE, _VOCAB_SIZE, arr)
            return torch.tensor(arr, dtype=torch.long).to(DEVICE)

        model = _get_mlp_model(place_id, race_type, course_len)
        with torch.no_grad():
            logits = model(_safe_id("father_cat"), _safe_id("mother_father_cat"),
                           _safe_id("paternal_gf_cat"), X_num)
...
def score_v7(full_df_64, place_id, race_type, course_len, final_odds, final_pop):
    try:
...
if os.path.exists(V13_CACHE):
    print(f"\nv13evalキャッシュ読み込み: {V13_CACHE}")
    with open(V13_CACHE, "rb") as f:
    ...
    for place_id in active_places:
        ...
        for cp in race_paths:
            ...
            full_df_64, v6_extra_df, v7_extra_df, v8_extra_df, pid, rtype, clen, hids, ok = \
                build_v8_row(race_id, rc_df, race_info_df, cp)
            if not ok: skipped += 1; continue

            s_v7  = score_v7(full_df_64, pid, rtype, clen, final_odds, final_pop)
            s_v13 = score_v13mlp(full_df_64, v6_extra_df, v7_extra_df, v8_extra_df,
                                  pid, rtype, clen, final_odds, final_pop, hids)
            if s_v13 is None: skipped += 1; continue
...
evaluate("v13mlp_no26（Embedding + MLP）",
         race_records,
         lambda r: r["s_v13"])

print(f"\n--- v13mlp × v7odds ブレンド αスイープ ---")
for alpha in [round(a * 0.1, 1) for a in range(0, 11)]:
    recs = [r for r in race_records if r["s_v7"] is not None]
    evaluate(f"v13×v7 α={alpha:.1f}", recs,
             lambda r, a=alpha: a * _norm(r["s_v13"]) + (1 - a) * _norm(r["s_v7"]))
