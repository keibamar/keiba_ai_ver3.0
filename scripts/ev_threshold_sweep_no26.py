"""
期待値フィルタリング 閾値スイープ（no26モデル・α=0.3固定）

blended = 0.3 × normalize(v6hit_extreme_no26) + 0.7 × normalize(v5prime_no26)
P_win   = softmax(blended)[本命]
EV      = P_win × 本命の単勝オッズ

EV閾値をスイープして、勝負レースを絞ったときの回収率変化を確認する。
収集データはキャッシュに保存し、2回目以降は即座に開始する。

実行:
    python scripts/ev_threshold_sweep_no26.py
"""
...
BET_UNIT      = 100
EVAL_YEAR     = 2026
EV_THRESHOLDS = [round(t * 0.1, 1) for t in range(5, 31)]  # 0.5〜3.0
CACHE_PATH    = os.path.join(PROJECT_ROOT, "logs", f"race_records_no26_{EVAL_YEAR}.pkl")
...
def score_extreme(full_df_64, place_id, race_type, course_len, final_odds, final_pop):
    try:
        ...
        model = _get_model(place_id, race_type, course_len, "_v6hit_extreme_no26")
...
if os.path.exists(CACHE_PATH):
    ...
    for place_id in active_places:
        ...
        for cp in race_paths:
            ...
            s_ext = score_extreme(full_df_64, pid, rtype, clen, final_odds, final_pop)
            ...
            odds_arr  = np.array([odds_map.get(u, np.nan) for u in umabans], dtype=float)
...
print(f"\n{'='*115}")
print(f"EV閾値スイープ  α={ALPHA}固定")
print(f"EV = softmax(blended)[本命] × 本命の単勝オッズ  (EV>1.0 が理論上プラス期待値)")
print(f"{'='*115}")
print(f"{'EV閾値':>8}  {'件数':>6}  {'単勝的中率':>10} {'単勝回収率':>10}  {'複勝的中率':>10} {'複勝回収率':>10}  {'3連複的中率':>11} {'3連複回収率':>11}")
print("-" * 115)

for ev_th in [None] + EV_THRESHOLDS:
    st = Stats()
    for r in race_records:
        blended = ALPHA * _norm(r["s_ext"]) + (1 - ALPHA) * _norm(r["s_v5p"])
        ...
        if ev_th is not None:
            honmei_odds = r["odds"][honmei_idx]
            if np.isnan(honmei_odds) or honmei_odds <= 0:
                continue
            ev = _softmax(blended)[honmei_idx] * honmei_odds
            if ev < ev_th:
                continue
    ...
    label = "全件(なし)" if ev_th is None else f"{ev_th:.1f}"
    print(f"  {label:>8}  {st.n:>6}件  "
