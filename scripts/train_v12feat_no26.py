"""v12feat_no26 モデル学習スクリプト（コーナー追走効率 + 上がり/タイムトレンド）

v8 データセット（98列）を使用:
  - v7 の 91列（v6 + kinryo/間隔/頭数/馬体重絶対値）
  - v8 追加の7列（corner_chase_1〜5 / agari_trend_5 / time_diff_trend_5）

タイム系の距離補正:
  - time_diff_trend_5: コース×距離別平均差のトレンドなので距離補正済み
  - agari_trend_5    : 上がり3F（最後の600m）は距離非依存

目的関数: binary + オッズ重み付き（v9feat/v11feat と同じ方式）
TRAIN_YEARS: 2020〜2025
サフィックス: _v12feat_no26

実行:
    python scripts/train_v12feat_no26.py
"""
...
from src.PredictionModels.LightGBM.make_dataset_v8 import (
    load_dataset_v8, make_dataset_for_train_v8,
)

TRAIN_YEARS   = list(range(2020, 2026))
TARGET_VENUES = list(range(1, 11))
MODEL_SUFFIX  = "_v12feat_no26"
N_TRIALS      = 20
    ...
    for place_id in TARGET_VENUES:
        ...
        for year in TRAIN_YEARS:
            missing = [
                ...
                if load_dataset_v8(place_id, year, rt, ln)[0].empty
            ]
            if not missing:
                continue
            print(f"\n[データ生成] {place_name} {year}年 ({len(missing)}コース欠損)")
            try:
                make_dataset_for_train_v8(place_id, year, vocab=vocab, course_filter=missing)
            except Exception:
...
if __name__ == "__main__":
    print("=" * 55)
    print(f"v12feat_no26  学習年: {TRAIN_YEARS[0]}〜{TRAIN_YEARS[-1]}")
    print("特徴量: v8（98列 = v7 + コーナー追走効率×5 + 上がりトレンド + タイム差トレンド）")
