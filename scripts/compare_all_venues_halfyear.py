"""
2026年上期 全場 v1 / v3blend / v4 的中率・回収率 一括集計

実行:
    python scripts/compare_all_venues_halfyear.py

- v1      : race_card CSV の rank_hitrate
- v3blend : v3モデル(64特徴量) + race_result 最終オッズでブレンド
- v4      : v4モデル(66特徴量) の生スコアで順位付け
            ※v3/v4モデルが存在しないコースはそのレースをスキップ
"""
...
BET_UNIT = 100
MODELS   = ["v1", "v3blend", "v4"]


# ---------- モデルロード ----------

def get_v4_model(place_id, race_type, length):
    type_str = "turf" if race_type == "芝" else "dirt"
    mp = os.path.join(paths.PREDICTION_MODEL_PATH, PLACE_LIST[place_id - 1],
                      f"{type_str}{length}_lambdarank_model_v4.txt")
...
def build_v3_features(race_id, rc_df, race_info_df):
    try:
        ...
        y_pred_v3 = model_v3.predict(full_df, num_iteration=model_v3.best_iteration)
        return full_df, y_pred_v3, place_id, race_type, course_len, True
...
def v4_ranks(full_df_64, place_id, race_type, course_len,
             final_odds_series, final_pop_series):
    try:
        odds_arr = final_odds_series.astype(float).values
        ...
        model  = get_v4_model(place_id, race_type, course_len)
...
# 2026年でrace_cardがある場(1-10)を自動検出
active_places = []
...
print("=" * 85)
print("2026年上期 的中率・回収率集計  全場 v1 / v3blend / v4  (¥100/点)")
...
for place_id in active_places:
    ...
    for cp in race_paths:
        ...
        # v1
        v1_col  = next((c for c in ["rank_hitrate", "rank"] if c in rc_df.columns), None)
        v1_top5 = []
        if v1_col:
            v1_sorted = pd.to_numeric(rc_df[v1_col], errors="coerce").sort_values().index
            v1_top5   = [str(int(float(rc_df["馬番"].iloc[i]))) for i in v1_sorted[:5]]

        # v3特徴量ビルド (v3blend・v4共用)
        ...
        for mname, top5 in [("v1", v1_top5), ("v3blend", v3_top5), ("v4", v4_top5)]:
    ...
    print(f"  --- {place_name} 小計 ({place_stats[place_name]['v1'].n}R) ---")
    for m in MODELS:
