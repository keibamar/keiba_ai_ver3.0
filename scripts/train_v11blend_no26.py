"""train_v11blend_no26.py — v11特徴量(114列) + 統合EV回帰（単勝EV + 複勝EV）

目的関数: 統合EV回帰
  target = is_winner × tan_odds + FUKU_WEIGHT × is_top3 × fuku_payout
  - tan_odds: 単勝払戻オッズ（TAN_ODDS_CAP 上限）
  - fuku_payout: 複勝払戻オッズ（FUKU_ODDS_CAP 上限）

評価指標: バリデーション期間の複勝 ROI（Optuna / early-stopping 共通）
TRAIN_YEARS: 2020〜2025（リークなし）
サフィックス: _v11blend_no26

実行:
    python scripts/train_v11blend_no26.py
"""
...
def _build_fuku_lookup(place_id, years):
    """
    Returns:
        {race_id_str: {result_flag: fuku_payout_odds}}
...
def _make_combined_ev_target(flag_df, X_df, race_ids, fuku_lookup):
    """統合EV回帰ターゲット: 単勝EV + FUKU_WEIGHT × 複勝EV"""
    is_winner    = (flag_df["result_flag"] == 4).values
    result_flags = flag_df["result_flag"].values

    # 単勝 EV
    if "current_odds" in X_df.columns:
        tan_odds = X_df["current_odds"].values.copy()
        tan_odds = np.where((tan_odds > 0) & np.isfinite(tan_odds), tan_odds, 1.1)
        tan_odds = np.clip(tan_odds, 1.0, TAN_ODDS_CAP)
    else:
        tan_odds = np.full(len(flag_df), 1.1)

    tan_ev = np.where(is_winner, tan_odds, 0.0)

    # 複勝 EV (placed horses only)
    ...
    return tan_ev + FUKU_WEIGHT * fuku_ev
...
def make_fuku_roi_eval(race_ids_val, flags_val, fuku_lookup):
    """バリデーション期間の複勝シミュレーション ROI（Optuna / early-stopping 共通）"""
        ...
        for rid in unique_ids:
            ...
            if picked_flag in (2, 3, 4):
                ...
                if rid_str in fuku_lookup:
                    ...
                    total_fuku_pay += fuku_pay * 100.0  # オッズ → 円
    ...
    for place_id in TARGET_VENUES:
        ...
        for race_type, length in name_header.COURSE_LISTS[place_id - 1]:
            ...
            try:
                ...
                y_tr = _make_combined_ev_target(flag_tr, X_tr, rid_tr, fuku_lookup)
                y_va = _make_combined_ev_target(flag_va, X_va, rid_va, fuku_lookup)

                n_win    = int((flag_tr["result_flag"] == 4).sum())
                n_top3   = int((flag_tr["result_flag"] >= 2).sum())
                fuku_cov = sum(
                    1 for r in rid_tr if str(int(float(str(r)))) in fuku_lookup
                )
                fuku_pct = fuku_cov / max(n_win, 1) * 100
                print(f"    train={len(X_tr):,}行 / val={len(X_va):,}行"
                      f" / 1着={n_win} / top3={n_top3} / 複勝カバー率≈{fuku_pct:.0f}%")
...
if __name__ == "__main__":
    print("=" * 55)
    print(f"v11blend_no26  学習年: {TRAIN_YEARS[0]}〜{TRAIN_YEARS[-1]}")
    print("特徴量: v11（114列 = v10 + trainer/same_cond/ground_delta）")
    print(f"目的関数: 統合EV回帰（単勝EV + {FUKU_WEIGHT}×複勝EV）")
    print(f"  TAN_ODDS_CAP={TAN_ODDS_CAP}  FUKU_ODDS_CAP={FUKU_ODDS_CAP}")
