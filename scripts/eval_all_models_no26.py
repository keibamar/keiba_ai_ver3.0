"""eval_all_models_no26.py

no26系モデルの一括比較評価スクリプト（2026年・リークなし）

比較対象:
  - no26ブレンド α=0.3  (v5prime_no26 × v6hit_extreme_no26)
  - v7odds_binary_no26  (オッズ重み付き二値分類)
  - v8profit_no26       (回収率直接最適化カスタムLoss)
  - v7×v5prime ブレンド αスイープ
  - v8×v5prime ブレンド αスイープ

実行:
    python scripts/eval_all_models_no26.py
"""
...
BET_UNIT  = 100
EVAL_YEAR = 2026
CACHE_PATH = os.path.join(PROJECT_ROOT, "logs", f"race_records_all_models_{EVAL_YEAR}.pkl")
...
# 1. no26ブレンド α=0.3（ベースライン）
evaluate("no26ブレンド α=0.3（v5prime × v6hit_extreme）",
         race_records,
         lambda r: 0.3 * _norm(r["s_ext"]) + 0.7 * _norm(r["s_v5p"]))

# 2. v7odds_binary 単独
evaluate("v7odds_binary_no26（オッズ重み付き二値分類）",
         [r for r in race_records if r["s_v7"] is not None],
         lambda r: r["s_v7"])

# 3. v8profit 単独
evaluate("v8profit_no26（回収率直接最適化）",
         [r for r in race_records if r["s_v8"] is not None],
         lambda r: r["s_v8"])

print(f"\n--- v7odds × v5prime_no26 ブレンド αスイープ ---")
for alpha in [round(a * 0.1, 1) for a in range(0, 11)]:
    recs = [r for r in race_records if r["s_v7"] is not None]
    evaluate(f"v7×v5prime α={alpha:.1f}", recs,
             lambda r, a=alpha: a * _norm(r["s_v7"]) + (1 - a) * _norm(r["s_v5p"]))

print(f"\n--- v8profit × v5prime_no26 ブレンド αスイープ ---")
for alpha in [round(a * 0.1, 1) for a in range(0, 11)]:
    recs = [r for r in race_records if r["s_v8"] is not None]
    evaluate(f"v8×v5prime α={alpha:.1f}", recs,
             lambda r, a=alpha: a * _norm(r["s_v8"]) + (1 - a) * _norm(r["s_v5p"]))

print(f"\n--- v8profit × v7odds ブレンド αスイープ ---")
for alpha in [round(a * 0.1, 1) for a in range(0, 11)]:
    recs = [r for r in race_records if r["s_v7"] is not None and r["s_v8"] is not None]
    evaluate(f"v8×v7 α={alpha:.1f}", recs,
             lambda r, a=alpha: a * _norm(r["s_v8"]) + (1 - a) * _norm(r["s_v7"]))
