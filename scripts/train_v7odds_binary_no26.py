"""v7odds_binary_no26 モデル学習スクリプト（オッズ重み付き二値分類・リークなし）

LambdaRank（順位最適化）から、オッズ重み付き二値分類（回収率重視）に目的関数を変更。

  ラベル: relevance_grade=4（1着）→ 1、それ以外→ 0
  重み:   勝ち馬 = current_odds、負け馬 = 1.0
          → 高オッズの穴馬を当てたほうがモデルにとってメリットが大きくなる
  TRAIN_YEARS: 2020〜2025（2026除外・リークなし）
  split: 先頭80%=train / 後ろ20%=validation（時系列正順）
  サフィックス: _v7odds_binary_no26

実行:
    python scripts/train_v7odds_binary_no26.py
"""
...
MODEL_SUFFIX  = "_v7odds_binary_no26"
N_TRIALS      = 20
    ...
    for place_id in TARGET_VENUES:
        ...
        for race_type, length in name_header.COURSE_LISTS[place_id - 1]:
            ...
            try:
                ...
                pos_ratio = y_tr.mean()
                mean_win_odds = X_tr.loc[y_tr == 1, "current_odds"].replace(-1, np.nan).mean() if "current_odds" in X_tr.columns else float("nan")
...
if __name__ == "__main__":
    print("=" * 55)
    print(f"v7odds_binary_no26  学習年: {TRAIN_YEARS[0]}〜{TRAIN_YEARS[-1]}")
    print("目的関数: binary + オッズ重み付き（勝ち馬weight=オッズ）")
    print("=" * 55)
    train_all_courses()
    print("\n" + "=" * 55 + "\n完了\n" + "=" * 55)
