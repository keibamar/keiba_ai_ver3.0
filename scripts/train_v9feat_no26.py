"""v9feat_no26 モデル学習スクリプト（5走拡張特徴量 + オッズ重み付き二値分類）

v6 データセット（86列）を使用:
  - v4 の 67列（血統36 + 過去3走21 + rank_trend + weight_change + dist_change + jockey(2) + waku/umaban + odds/popularity）
  - v5 追加の血統カテゴリ3列
  - v6 追加の5走拡張16列（4走前・5走前の7列×2 + rank_trend_5 + win_rate_recent5）

目的関数: binary + オッズ重み付き（v7odds と同じ）
TRAIN_YEARS: 2020〜2025（リークなし）
サフィックス: _v9feat_no26

実行:
    python scripts/train_v9feat_no26.py
"""
...
from src.PredictionModels.LightGBM.make_dataset_v6 import (
    load_dataset_v6, make_dataset_for_train_v6,
)

TRAIN_YEARS   = list(range(2020, 2026))
TARGET_VENUES = list(range(1, 11))
MODEL_SUFFIX  = "_v9feat_no26"
N_TRIALS      = 20
    ...
    for place_id in TARGET_VENUES:
        ...
        for year in TRAIN_YEARS:
            missing = [
                ...
                if load_dataset_v6(place_id, year, rt, ln)[0].empty
            ]
            if not missing:
                continue
            print(f"\n[データ生成] {place_name} {year}年 ({len(missing)}コース欠損)")
            try:
                make_dataset_for_train_v6(place_id, year, vocab=vocab, course_filter=missing)
            except Exception:
            ...
            try:
                ...
                for year in TRAIN_YEARS:
                    for band_length in _band_lengths(place_id, race_type, length):
                        df, flag = load_dataset_v6(place_id, year, race_type, band_length)
...
if __name__ == "__main__":
    print("=" * 55)
    print(f"v9feat_no26  学習年: {TRAIN_YEARS[0]}〜{TRAIN_YEARS[-1]}")
    print("特徴量: v6（5走拡張 86列）")
