"""v15pace_no26 モデル学習スクリプト（ペース適性特徴量 + オッズ重み付き二値分類）

v9 データセット（105列）を使用:
  - v8 の 98列（血統・過去5走・コーナー追走・トレンド等）
  - v9 追加の 7列:
      agari_df_course_1〜5: 各走の上り3F とコース×馬場別平均上り3F との差（末脚相対力）
      corner_ratio_std5: 過去5走の位置取り標準偏差（脚質安定性）
      agari_std5: 過去5走の上り3F 標準偏差（末脚安定性）

目的関数: binary + オッズ重み付き（v7odds と同じ）
TRAIN_YEARS: 2020〜2025（リークなし）
サフィックス: _v15pace_no26

実行: python scripts/train_v15pace_no26.py
"""
...
if __name__ == "__main__":
    print("=" * 55)
    print(f"v15pace_no26  学習年: {TRAIN_YEARS[0]}〜{TRAIN_YEARS[-1]}")
    print("データセット: v9（105列: v8 + ペース適性7列）")
