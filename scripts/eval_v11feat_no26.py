"""eval_v11feat_no26.py

v11feat_no26（91列特徴量 = v6+間隔/斤量/頭数/馬体重絶対値）を既存モデルと比較評価。

v9eval キャッシュ (race_records_v9eval_2026.pkl) を流用し、
v11feat スコアを追加した新キャッシュを作成する。

実行:
    python scripts/eval_v11feat_no26.py
"""
...
def _get_current_date_from_path(race_card_path):
    """data/race_card/20260104/race_id.csv → pd.Timestamp('2026-01-04')"""
    try:
        dir_name = os.path.basename(os.path.dirname(race_card_path))  # "20260104"
        return pd.to_datetime(dir_name, format="%Y%m%d")
    except Exception:
        return pd.NaT


def build_v7_features(race_id, rc_df, race_info_df, race_card_path):
    """v7特徴量（90列）を構築する。

    Returns:
        full_df_64    : v3_base(60) + jockey(2) + waku/umaban(2) = 64列
        v6_extra_df   : 16列
        v7_extra_df   : 5列（kinryo/days_since_last_race/n_horses_today/n_horses_1/hw_abs_1）
        place_id      : int
        race_type     : str
        course_len    : str
        ok            : bool
    ...
    try:
        ...
        # v3 base (60 cols) + v6 extra (16 cols) + v7 extras (per horse)
        ...
        return full_df_64, dataset_v6_extra, v7_extra_df, place_id, race_type, course_len, True
    except Exception as e:
        prediction_error(e)
        return None, None, None, None, None, None, False


def score_v11feat(full_df_64, v6_extra_df, v7_extra_df,
                  place_id, race_type, course_len, final_odds, final_pop, horse_ids):
    """v11feat_no26: v7特徴量(90列)でスコアリング。
    列順: v3_base(60)+jockey(2)+waku/umaban(2)+odds(1)+pop(1)+ped_cat(3)+v6_extra(16)+v7_extra(5)
    """
    try:
        ...
        model = _get_model(place_id, race_type, course_len, "_v11feat_no26")
...
if os.path.exists(V11_CACHE):
    ...
    print(f"  {len(v9_records)}R → v11featスコアを追加します")

    # v9キャッシュには s_v5p, s_ext, s_v7, s_v9 が格納済み
    # ここでは v7特徴量（91列）を別途構築して v11feat スコアを追加する
    ...
    for place_id in active_places:
        ...
        for cp in race_paths:
            ...
            full_df_64, v6_extra_df, v7_extra_df, pid, rtype, clen, ok = \
                build_v7_features(race_id, rc_df, race_info_df, cp)
            ...
            s_v11 = score_v11feat(full_df_64, v6_extra_df, v7_extra_df, pid, rtype, clen,
                                  final_odds, final_pop, rc_df["horse_id"].tolist())
            if s_v11 is None: skipped += 1; continue
...
# v9feat 単独
evaluate("v9feat_no26（5走拡張・前回最良との比較）",
         [r for r in race_records if r["s_v9"] is not None],
         lambda r: r["s_v9"])

# v11feat 単独
evaluate("v11feat_no26（+間隔/斤量/頭数/馬体重）",
         race_records,
         lambda r: r["s_v11"])

print(f"\n--- v11feat × v7odds ブレンド αスイープ ---")
for alpha in [round(a * 0.1, 1) for a in range(0, 11)]:
    recs = [r for r in race_records if r["s_v7"] is not None]
    evaluate(f"v11×v7 α={alpha:.1f}", recs,
             lambda r, a=alpha: a * _norm(r["s_v11"]) + (1 - a) * _norm(r["s_v7"]))

print(f"\n--- v11feat × v9feat ブレンド αスイープ ---")
for alpha in [round(a * 0.1, 1) for a in range(0, 11)]:
    recs = [r for r in race_records if r["s_v9"] is not None]
    evaluate(f"v11×v9 α={alpha:.1f}", recs,
             lambda r, a=alpha: a * _norm(r["s_v11"]) + (1 - a) * _norm(r["s_v9"]))
