...
<body>
  ...
  <ul>
    ...
    <li><strong>オッズ</strong>：市場のオッズ情報（回収率最適化に活用）</li>
  ...
  <h3>MARモデルファミリーについて</h3>
  <p>当サイトでは、目的関数の異なる3種類のモデルを <strong>MARファミリー</strong> として運用しています。</p>
  <ul>
    <li><strong>MAR</strong>（メインモデル）：的中率と回収率を総合的に評価するバランス型。出馬表の「MAR」列がこのスコアです。</li>
    <li><strong>MAR-hit</strong>（的中率重視）：より高い確率で本命馬が馬券に絡むことを優先して学習したモデル。</li>
    <li><strong>MAR-val</strong>（回収率重視）：高配当・高回収率を狙う本命選定を優先して学習したモデル。</li>
  </ul>
  <p>3モデルの成績はすべて<a href="performance/index.html">AI成績レポート</a>で公開しています。</p>
